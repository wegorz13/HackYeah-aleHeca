import { useEffect, useState } from "react";

type SecondStepProps = {
    chosenTraits: string[];
    setChosenTraits: (traits: string[]) => void;
};

export const SecondStep = ({ chosenTraits, setChosenTraits }: SecondStepProps) => {
    const [traits, setTraits] = useState([]);

    function toggleTrait(trait: string) {
        if (chosenTraits.length === 0) {
            setChosenTraits((prevState) => [...prevState, trait]);
        } else {
            if (chosenTraits.includes(trait)) {
                const filtered = chosenTraits.filter((t) => t !== trait);
                setChosenTraits(filtered);
            } else {
                setChosenTraits((prevState) => [...prevState, trait]);
            }
        }
    }

    useEffect(() => {
        fetch("http://localhost:3000/traits")
            .then((res) => res.json())
            .then((data) => setTraits(data))
            .catch((err) => console.error("Błąd przy pobieraniu:", err));
    }, []);

    return (
        <div>
            <div className="flex w-full flex-col items-start gap-1">
                <b className="text-left text-lg text-sm">I’m looking for</b>
                <p className="text-left text-xs text-gray-600">Choose up to 5 more</p>
            </div>
            <div className="grid w-full grid-cols-3 gap-2">
                {traits.map((trait, index) => (
                    <button
                        key={index}
                        onClick={() => toggleTrait(trait.name)}
                        className={`m-1 rounded-xl px-2 py-1 text-center text-sm transition-colors duration-200 ${
                            chosenTraits.includes(trait.name) ? "bg-orange-500 text-white" : "bg-orange-100 text-black"
                        }`}
                    >
                        {trait.name}
                    </button>
                ))}
            </div>
        </div>
    );
};
